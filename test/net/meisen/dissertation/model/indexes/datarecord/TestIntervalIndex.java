...
public class TestIntervalIndex extends DbBasedTest {
    ...
    private TidaModel loadModel(final String dbName, final String dbPath,
        ...
        // load the model
        return loader.loadViaXslt("/net/meisen/dissertation/model/indexes/datarecord/" + modelPath);
    }
     ...
     *
     * @throws IOException if the db cannot be loaded
     */
    @Test
    ...
    public void testIndexWithNullValuesBoundaries() throws IOException {
        ...
        for (short i = (short) idx.getNormStart(); i < idx.getNormEnd(); i++) {
            ...
            final SliceWithDescriptors<Short> slice = (SliceWithDescriptors<Short>) idx.getSliceById(i);
     ...
     *
     * @throws IOException if a file cannot be read or created
     */
    @Test
    ...
    private void assertIntervalsWithDescriptors(final TidaModel model,
        ...
        for (int i = 0; i < res.length; i++) {
            ...
            final Set<Descriptor<?, ?, ?>> notExpected = new HashSet<>();
            notExpected.add(nul);
