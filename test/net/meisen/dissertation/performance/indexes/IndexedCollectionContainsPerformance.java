 ...
 * 
 */
public class IndexedCollectionContainsPerformance {
	 ...
	 * test-implementation.
	 */
	public void runContainsPerformanceTest() {
		for (int i = 0; i < amountOfRuns; i++) {
			runSingleContainsPerformanceTest();
		}
	}
	 ...
	 */
	protected void runSingleContainsPerformanceTest() {
		...
		for (final Entry<String, BaseIndexedCollection> entry : indexes.entrySet()) {
			runIndexContainsTest(entry.getKey(), entry.getValue(), ids, values);
		}
	...
	protected void runIndexContainsTest(final String name, final Object idx,
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (isIndex) {
				final Object d = values[i];
				assertTrue(((BaseIndexedCollection) idx).containsObject(d));
			} else if (isMap) {
				final Object id = ids[i];
				assertTrue(((Map<Object, Object>) idx).containsKey(id));
			}
		}
