...
public class TestTidaResultSet extends TestBaseForConnections {
    ...
    public void testResultSetTimeout() throws SQLException {
        ...
        exception = null;
        try {
            stmt.executeQuery("SELECT TIMESERIES FROM testNumberModel");
        } catch (final SQLException e) {
            exception = e;
        }
        assertNotNull(exception);
        ...
        assertEquals(1, manager.sizeOfScopes());
        assertEquals(1, manager.sizeOfProtocols(tStmt));
        assertEquals(0, manager.sizeOfProtocols(null));
        assertFalse(manager.isOwner(null));
        assertTrue(manager.isOwner(tStmt));
     ...
     *
     * @throws SQLException an unexpected exception
     */
    @Test
    ...
    public void testMultipleRecordsSelectsOnOneStatement() throws SQLException {
        ...
        for (int i = 0; i < 1000; i++) {
            res = stmt.executeQuery("SELECT RECORDS FROM testNumberModel WITHIN [1,3]");
            res.close();
        }

        res = stmt.executeQuery("SELECT RECORDS FROM testNumberModel WITHIN [2,3]");
        res.close();
