...
public class TestNashornScriptPreProcessor extends LoaderBasedTest {
    ...
    public void testUsageInModel() {
        final TidaModel model = m("/net/meisen/dissertation/impl/dataintegration/testNashornScriptModel.xml", true);
        ...
        assertEquals(4, record.getPosition("LOCATION"));
        assertEquals(5, record.getPosition("MORESCREAMS"));
        assertEquals(6, record.getPosition("PERSON"));
        assertEquals(7, record.getPosition("CONST"));
        assertEquals(8, record.getPosition("SCREAMS"));
        assertEquals(9, record.getPosition("SCREAMCATEGORY"));

        assertEquals("[ID]", record.getName(1));
        assertEquals("[START]", record.getName(2));
        assertEquals("[END]", record.getName(3));
        assertEquals("LOCATION", record.getName(4));
        assertEquals("MORESCREAMS", record.getName(5));
        assertEquals("PERSON", record.getName(6));
        assertEquals("CONST", record.getName(7));
        assertEquals("SCREAMS", record.getName(8));
        assertEquals("SCREAMCATEGORY", record.getName(9));

        assertEquals(0, record.getValue(1));
        assertTrue(record.getValue(2) instanceof Date);
        assertTrue(record.getValue(3) instanceof Date);
        assertEquals("Aachen", record.getValue(4));
        assertEquals(1, record.getValue(5));
        assertEquals("Tobias", record.getValue(6));
        assertEquals("MYCONSTANT", record.getValue(7));
        assertEquals(0, record.getValue(8));
        assertEquals("NONE", record.getValue(9));
    }
     ...
     *
     * @param script the script to be bound
    ...
    protected ScriptPreProcessor create(final String script) {
        ...
        final ScriptPreProcessorConfig config = new ScriptPreProcessorConfig("nashorn");
