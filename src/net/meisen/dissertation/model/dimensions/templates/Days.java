...
/**
 * A template for a {@code Days}-level.
 * 
 * @author pmeisen
 * 
 */
public class Days extends BaseTimeLevelTemplate {
		...
		if (granularity instanceof IDateBasedGranularity == false) {
			throw new ForwardedRuntimeException(TimeDimensionException.class,
					1000, granularity.getClass().getSimpleName(), getClass()
							.getSimpleName());
		} else if (!((IDateBasedGranularity) granularity).isAssignableTo('d')) {
			throw new ForwardedRuntimeException(TimeDimensionException.class,
					1000, granularity.getClass().getSimpleName(), getClass()
							.getSimpleName());
		}
	...
	public Iterator<TimeLevelMember> it(final IntervalModel model,
		...
		validate(model);
		return createIterator(model, timezone, Day.instance());
	}
	...
	public Iterator<TimeLevelMember> it(final IntervalModel model,
			...
			throws ForwardedRuntimeException {
		validate(model);
		return createIterator(model, timezone, Day.instance(), start, end);
	}
}
